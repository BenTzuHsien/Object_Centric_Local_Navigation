...
from Object_Centric_Local_Navigation.baseline.single_step_dataset import SingleStepDataset
...
def train_single_step(model, dataset_paths, evaluation_path, result_path, num_gpus=1, start_index=1):
    ...
    for epoch in training_bar:
        ...
        running_loss = 0.0
        for current_embedding, goal_embedding, action, prompt in tqdm(train_dataloader, desc="Training", leave=False):
...
if __name__ == '__main__':
    ...
    from Object_Centric_Local_Navigation.baseline.dinotxt_mlp5_uni import DinotxtMlp5Uni
    model = DinotxtMlp5Uni(use_embeddings=True)
